...
function alphaSetConfigMode(mode){alphaConfigMode=mode;document.getElementById("config-mode-simple")?.classList.toggle("active",mode==="simple");document.getElementById("config-mode-advanced")?.classList.toggle("active",mode==="advanced");document.querySelectorAll("#config-builder details:nth-child(n+4)").forEach(d=>d.hidden=mode==="simple");}
function alphaConfigTags(v){try{return Array.isArray(v)?v:JSON.parse(v||"[]")}catch(_){return[]}}
...
function alphaToggleRepoSource(type,source,btn){
  const local=document.getElementById(`config-${type}-local`),suggested=document.getElementById(`config-${type}-suggested`);
  if(local)local.classList.toggle("hidden",source!=="local"); if(suggested)suggested.classList.toggle("hidden",source!=="suggested");
  btn?.parentElement?.querySelectorAll("button").forEach(x=>x.classList.toggle("active",x===btn));
  if(source==="suggested")alphaLoadZeusSuggestions(type);
}
function alphaInitZeusCountrySelect(){const sel=document.getElementById("zeus-proxy-country");if(!sel||sel.options.length)return;sel.innerHTML=alphaZeusCountries.map(c=>`<option value="${c}">${c==="ALL"?"همه کشورها":c}</option>`).join("");sel.value="ALL"}
async function alphaLoadZeusSuggestions(type){
  try{
    alphaInitZeusCountrySelect();
    const country=type==="proxy"?(document.getElementById("zeus-proxy-country")?.value||"ALL"):"";
    const d=await api(`/api/config/suggestions?type=${type}&limit=24${country?`&country=${encodeURIComponent(country)}`:""}`);
    const list=document.getElementById(`config-${type}-suggested-list`);if(!list)return;
    const items=d.items||[]; alphaZeusSuggested[type]=items;
    list.innerHTML=items.map((x,i)=>{const label=type==="ip"?x.address:`${x.host}:${x.port}`;const sub=type==="ip"?"IP · ZEUS":`${x.type} · ${x.country||"ALL"} · ZEUS`;return `<div class="config-suggestion-item"><div><b>${alphaEsc(label)}</b><small>${alphaEsc(sub)}</small></div><button class="primary small" onclick="alphaImportZeusSuggestion('${type}',${i})">استفاده</button></div>`}).join("")||'<div class="empty-state">مورد پیشنهادی پیدا نشد.</div>';
  }catch(e){const el=document.getElementById(`config-${type}-suggested-list`);if(el)el.innerHTML=`<div class="empty-state">${alphaEsc(e.message||"خطا در دریافت پیشنهادها")}</div>`}
}
async function alphaImportZeusSuggestion(type,index){const item=alphaZeusSuggested[type]?.[index];if(!item)return;try{const r=await api("/api/config/suggestions/import",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({type,item})});await alphaLoadConfigRepo(type);alphaSelectConfigRepo(type,r.id);alphaToast(r.existing?"این مورد قبلاً در مخزن وجود داشت و انتخاب شد":"مورد پیشنهادی به مخزن اضافه و انتخاب شد","success")}catch(e){alphaToast(e.message||"وارد کردن پیشنهاد ناموفق بود","error")}}
async function alphaLoadConfigFactory(){alphaInitZeusCountrySelect();await Promise.all([alphaLoadConfigRepo("ip"),alphaLoadConfigRepo("proxy"),alphaLoadConfigUsers(),alphaLoadConfigTemplates(),alphaLoadConfigSnapshots()]);alphaSetConfigMode(alphaConfigMode)}
async function alphaLoadConfigRepo(type){try{const q=encodeURIComponent(cfgVal(type==="ip"?"config-ip-search":"config-proxy-search"));const r=await api(`/api/config/${type==="ip"?"ip-repository":"proxy-repository"}?q=${q}`);if(type==="ip")alphaConfigIPs=r.items||[];else alphaConfigProxies=r.items||[];alphaRenderConfigRepo(type)}catch(e){const el=document.getElementById(type==="ip"?"config-ip-list":"config-proxy-list");if(el)el.innerHTML=`<div class="empty-state">${alphaEsc(e.message||"خطا")}</div>`}}
function alphaRenderConfigRepo(type){const items=type==="ip"?alphaConfigIPs:alphaConfigProxies,el=document.getElementById(type==="ip"?"config-ip-list":"config-proxy-list");if(!el)return;el.innerHTML=items.map(x=>{const selected=(type==="ip"?cfgVal("cfg-ip-id"):cfgVal("cfg-proxy-id"))===String(x.id);const tags=alphaConfigTags(x.tags);return `<button class="config-repo-item ${selected?"selected":""}" onclick="alphaSelectConfigRepo('${type}','${alphaEsc(x.id)}')"><span class="repo-main"><b>${alphaEsc(type==="ip"?x.address:x.host+":"+x.port)}</b><small>${alphaEsc(type==="ip"?(x.country||"Unknown"):(x.type+" · "+(x.country||"Unknown")))}</small></span><span class="repo-side"><i class="repo-dot ${x.status==='active'?'on':'off'}"></i><small>${x.latency_ms!=null?x.latency_ms+"ms":"—"}</small></span>${tags.length?`<span class="repo-tags">${tags.slice(0,3).map(t=>`<em>${alphaEsc(t)}</em>`).join("")}</span>`:""}</button>`}).join("")||'<div class="empty-state">موردی در مخزن وجود ندارد.</div>'}
function alphaSelectConfigRepo(type,id){const arr=type==="ip"?alphaConfigIPs:alphaConfigProxies,x=arr.find(v=>String(v.id)===String(id));if(!x)return;let hidden=document.getElementById(type==="ip"?"cfg-ip-id":"cfg-proxy-id");if(!hidden){hidden=document.createElement("input");hidden.type="hidden";hidden.id=type==="ip"?"cfg-ip-id":"cfg-proxy-id";document.getElementById("config-builder")?.appendChild(hidden)}hidden.value=x.id;if(type==="ip"){document.getElementById("cfg-ip-display").value=`${x.address}${x.port?":"+x.port:""} · ${x.country||"Unknown"}`;if(x.port)document.getElementById("cfg-port").value=x.port}else{document.getElementById("cfg-proxy-display").value=`${x.host}:${x.port} · ${x.type}`}alphaRenderConfigRepo(type);alphaPreviewConfig(true)}
async function alphaLoadConfigUsers(){try{const r=await api("/api/users?limit=100&page=1"),items=r.items||r;const sel=document.getElementById("cfg-user");if(sel)sel.innerHTML='<option value="">انتخاب کاربر...</option>'+items.map(x=>`<option value="${alphaEsc(x.id)}">${alphaEsc(x.username)} · ${alphaEsc(x.protocol||"VLESS")}</option>`).join("")}catch(e){}}
async function alphaLoadConfigTemplates(){try{const r=await api("/api/config/templates"),items=r.items||[];alphaConfigTemplates=items;const sel=document.getElementById("cfg-template");if(sel)sel.innerHTML='<option value="">بدون Template</option>'+items.map(x=>`<option value="${alphaEsc(x.id)}">${alphaEsc(x.name)}</option>`).join("");const box=document.getElementById("config-presets");if(box)box.innerHTML=items.slice(0,6).map(x=>`<button onclick="alphaApplyTemplate('${alphaEsc(x.id)}')">${alphaEsc(x.name)}</button>`).join("")}catch(e){}}
async function alphaLoadConfigSnapshots(){try{const r=await api("/api/config/snapshots"),items=r.items||[];alphaConfigSnapshots=items;const box=document.getElementById("config-snapshot-list");if(box)box.innerHTML=items.slice(0,8).map(x=>`<div class="config-snapshot-row"><div><b>${alphaEsc(x.name)} <small>v${x.version}</small></b><span>${new Date(Number(x.created_at)).toLocaleString("fa-IR")}</span></div><button class="ghost small" onclick="alphaRestoreSnapshot('${alphaEsc(x.id)}')">مشاهده</button></div>`).join("")||'<div class="empty-state">هنوز Snapshotی ساخته نشده.</div>'}catch(e){}}
function alphaApplyTemplate(id){if(!id)return;const t=alphaConfigTemplates.find(x=>String(x.id)===String(id));if(!t)return;let s={};try{s=JSON.parse(t.settings_json||"{}")}catch(_){};for(const [k,v] of Object.entries(s)){const el=document.getElementById("cfg-"+k);if(el!=null)el.value=v}alphaToast(`Template «${t.name}» اعمال شد`,'success');alphaPreviewConfig(true)}
function alphaConfigPayload(){let settings={name:cfgVal("cfg-name"),protocol:cfgVal("cfg-protocol"),port:Number(cfgVal("cfg-port")||443),transport:cfgVal("cfg-transport"),security:cfgVal("cfg-security"),sni:cfgVal("cfg-sni"),fp:cfgVal("cfg-fp"),path:cfgVal("cfg-path"),host:cfgVal("cfg-host"),flow:cfgVal("cfg-flow"),note:cfgVal("cfg-note"),use_proxy:document.getElementById("cfg-use-proxy")?.checked||false};return {...settings,user_id:cfgVal("cfg-user"),ip_id:document.getElementById("cfg-ip-id")?.value||"",proxy_id:document.getElementById("cfg-proxy-id")?.value||"",template_id:cfgVal("cfg-template")}}
async function alphaValidateConfig(silent=false){try{const d=await api("/api/config/generate",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(alphaConfigPayload())});const box=document.getElementById("config-validation");if(d.valid){if(box)box.innerHTML='<span class="ok-text">✓ تنظیمات معتبر است و آماده ساخت Snapshot می‌باشد.</span>';if(!silent)alphaToast("اعتبارسنجی موفق بود","success");return d}if(box)box.innerHTML=`<span class="bad-text">! ${alphaEsc((d.errors||[]).join(" · "))}</span>`;if(!silent)alphaToast((d.errors||[]).join(" · "),"error");return d}catch(e){if(!silent)alphaToast(e.message||"اعتبارسنجی ناموفق بود","error");return {valid:false}}}
async function alphaPreviewConfig(silent=false){const d=await alphaValidateConfig(true);const out=document.getElementById("config-output"),meta=document.getElementById("config-preview-meta"),status=document.getElementById("config-preview-status");if(!d.valid){if(out)out.textContent="تنظیمات کامل نیست.";if(status){status.textContent="INVALID";status.className="badge danger"}return}if(out)out.textContent=d.config||"";if(meta)meta.innerHTML=`<div><span>IP</span><b>${alphaEsc(d.ip?.address||"—")}</b></div><div><span>Proxy</span><b>${alphaEsc(d.proxy?d.proxy.host+":"+d.proxy.port:"بدون Proxy")}</b></div><div><span>Protocol</span><b>${alphaEsc(d.settings?.protocol||"—")}</b></div><div><span>Transport</span><b>${alphaEsc(d.settings?.transport||"—")}</b></div>`;if(status){status.textContent="VALID";status.className="badge active"}if(!silent)alphaToast("پیش‌نمایش آماده شد","success");return d}
async function alphaCreateConfigSnapshot(){const payload=alphaConfigPayload();if(!payload.name)payload.name="ALPHA Config";const d=await alphaValidateConfig(true);if(!d.valid)return alphaToast("ابتدا خطاهای تنظیمات را برطرف کن","error");try{if(document.getElementById("cfg-save-template")?.checked){await api("/api/config/templates",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:payload.name,mode:alphaConfigMode,settings:payload})});await alphaLoadConfigTemplates()}const r=await api("/api/config/snapshots",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});document.getElementById("config-output").textContent=r.config||d.config;alphaToast(`Snapshot نسخه ${r.version} ساخته شد`,"success");await alphaLoadConfigSnapshots()}catch(e){alphaToast(e.message||"ساخت Snapshot ناموفق بود","error")}}
function alphaRestoreSnapshot(id){const x=alphaConfigSnapshots.find(v=>String(v.id)===String(id));if(!x)return;let s={};try{s=JSON.parse(x.settings_json||"{}")}catch(_){};for(const [k,v] of Object.entries(s)){if(k==="ip_snapshot"||k==="proxy_snapshot")continue;const el=document.getElementById("cfg-"+k);if(el!=null&&typeof v!=="object")el.value=v}document.getElementById("config-output").textContent=x.config_text||"";alphaToast(`Snapshot ${x.name} v${x.version} بارگذاری شد`,'success')}
function alphaAddRepoItem(type){const title=type==="ip"?"افزودن IP":"افزودن Proxy";const address=prompt(type==="ip"?"IP Address":"Proxy Host");if(!address)return;let port=Number(prompt("Port",type==="ip"?"443":"8080"));if(!port)return;const body=type==="ip"?{address,port,country:prompt("Country","Unknown")||"Unknown",source:"manual",status:"active"}:{host:address,port,type:(prompt("Proxy Type","HTTP")||"HTTP").toUpperCase(),country:prompt("Country","Unknown")||"Unknown",source:"manual",status:"active"};api(`/api/config/${type==="ip"?"ip-repository":"proxy-repository"}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)}).then(()=>{alphaToast(`${title} ثبت شد`,'success');alphaLoadConfigRepo(type)}).catch(e=>alphaToast(e.message||"ثبت ناموفق بود","error"))}
